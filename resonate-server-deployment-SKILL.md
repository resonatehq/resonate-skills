...
Flags:
  --system-url string              Public URL for the server
...
WorkingDirectory=/var/lib/resonate
ExecStart=/usr/local/bin/resonate serve \
  --system-url https://resonate.example.com \
...
The `--system-url` flag tells the Resonate server its public address. This is critical when:
...
2. **Missing --system-url on server**
   ```bash
   # Server doesn't know its public URL
   # Update service with --system-url flag
   ```
...
3. Configure `--system-url` for public accessibility
...
**Critical flags:**
- `--system-url`: Server's public URL (required for external access)
