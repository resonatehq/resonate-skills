...
// Dispatch a workflow and wait for the result
console.log("Worker running...");

const result = await resonate.run("greet-demo", "greet", "World");
...
2. **`resonate.run()`** created a promise (`greet-demo`) and a task on the server, then waited for the result
...
resonate.register("approval", function* (ctx: Context, orderId: string) {
  // Create the promise — the server assigns its id
...
Resolve externally (from an HTTP handler, webhook, or script) — use the id the worker logged:
```typescript
