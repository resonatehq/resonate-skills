...
function* bookTrip(ctx: Context, tripId: string) {
  ...
  try {
    ...
    // Car rental: fail fast so compensation runs immediately on unavailability
    const carId = yield* ctx.run(
      bookCarRental,
      tripId,
      ctx.options({ retryPolicy: noRetry }),
    );
    return { status: "success", flightId, hotelId, carId };
  } catch (error) {
...
**When to use no-retry vs default retry in sagas:**

| Step type | Retry policy | Reason |
|-----------|-------------|--------|
| External booking API (non-idempotent) | `noRetry` | Don't double-book; compensate instead |
| Idempotent DB write | Default | Safe to retry — won't create duplicates |
